...
<script type="text/javascript">
   function check()
   {
      ...
      //URL 준비 (여기서 바로 submit하는게 아님 ajax에 url넘겨줘야한다)
   ...
   //아이디 중복 검사의 결과 (1or0)를 통보받아
   //사용자에게 메세지 출력 -> span -> result
   function callBack()
   {
      //외부로 추출된 업무 처리 내용 구성(responseText / responseXML)
      
      var data = ajax.responseText; // 어떤 형태로 받는지는 ajax controller에 따라 달림 
   ...
   function callBack2()
   {
      ...
      //모든 『<item>』 엘리먼트 반환받기 
      var itemList = root.getElementsByTagName("item"); //item들
   ...
   function selectZipCode(sel)
   {
	   ...
	   if(sel.value != "0")
	   {
		   // 06253/서울특별시 강남구 강남대로 298 (역삼동 838 푸른덴셜)
