...
public class MyUtil
{
	...
	public String pageIndexList(int currentPage, int totalPage, String listUrl)
	{
		...
		int page;	//-- 몇 페이지 인지
		int n;		//-- 점프하려고 있는 변수 6 에서 next 누르면 16에 있게
		...
		*/
		...
		if(listUrl.indexOf("?") != -1)  // 링크를 설정할 ULR 에 『?』가 들어있으면   (indexOf 는 찾는 문자열이 없으면 -1을 리턴)
		{
		...
		// 1 페이지 (가 있어야 하는 상황 조건)
		if( (totalPage>numPerBlock) && (currentPageSetup>0) )
		{
			//				-- 숫자 사이사이 공백을 위해 한칸 띄움
			strList.append(" <a href='" + listUrl + "pageNum=1'>1</a>");
		}
		//-- listUrl 은 위에서 (라인 77) 이미 천저리가 끝난 상황이기 때문에
		//   『...?』 상태 또는 『...?...&』 인 상태이다.
		//   이로 인해 결과는 
		...
		if ( (totalPage>numPerBlock) && (currentPageSetup>0) )
		{
			strList.append(" <a href='" + listUrl + "pageNum=" + n + "'>1</a>");
		}
